...
//        String coAddr = businessRegistration.getCoAddr();
//        String corp = businessRegistration.getCorp();
        if (!StringUtils.isEmpty(coName)) {
            wrapper.like("co_name", coName);
        }

        businessRegistrationService.page(pageBusiness, wrapper);
        long total = pageBusiness.getTotal();
        List<BusinessRegistration> records = pageBusiness.getRecords();
        return commonResult.OK().data("total", total).data("result", records);
    }
}
